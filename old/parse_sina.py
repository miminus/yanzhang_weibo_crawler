...
post_con_pattern=re.compile(r'(<dl .*?action-type="feed_list_item".*?class="clear">\s*</dd>\s*</dl>)',re.S)
...
main_usr_pattern=re.compile(r'<a .*?href="(http://weibo.com/.*?)"',re.S)
poster_pattern=re.compile(r'title="(.*?)"',re.S)
user_image_pattern=re.compile(r'src="(http://.*?)"',re.S)
user_id_pattern=re.compile(r'id=(\d+)',re.S)
...
for d in post_con:
	...
	#aa=aa[0]
	aa=aa[0]
	#aa=re.findall(content_pp,aa[0].encode('gbk'))
	#print aa[0]
	...
	#############时间#######################
	post_time=soup.find_all("a",class_="date")
